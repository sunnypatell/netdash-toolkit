...
export function IPv6RangesPanel({ searchTerm }: { searchTerm: string }) {
  const rows = useMemo(
    () => filterRows(IPV6_SPECIAL_RANGES, searchTerm, (row) => searchableText(columns, row)),
    [searchTerm]
  )
  ...
  return (
    <ReferenceTable
      ...
      description={`Special IPv6 prefixes from the IANA IPv6 special-purpose registry and RFC 4291 (${rows.length} shown)`}
      rows={rows}
      columns={columns}
