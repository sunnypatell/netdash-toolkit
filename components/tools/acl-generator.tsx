...
export function ACLGenerator() {
  ...
  const generateJuniperFirewallFilter = (): string => {
    const timestamp = new Date().toISOString()
    const rules = aclType === "standard" ? standardRules : extendedRules
    let output = ""

    output += `/* ${aclType.toUpperCase()} Firewall Filter ${aclName} - Generated by Network Toolbox */\n`
    output += `/* Generated on: ${timestamp} */\n`
    output += `/* Total terms: ${rules.length} */\n\n`

    output += `firewall {\n`
    output += `    family inet {\n`
    output += `        filter ${aclName} {\n`

    rules.forEach((rule, index) => {
      output += `            /* ${rule.description || `Rule ${index + 1}`} */\n`
      output += `            term rule-${index + 1} {\n`

      if (aclType === "standard") {
        const stdRule = rule as StandardACLRule
        output += `                from {\n`
        if (stdRule.sourceNetwork !== "any") {
          try {
            const parsed = parseACLNetwork(stdRule.sourceNetwork)
            if (parsed.kind === "host") {
              output += `                    source-address ${parsed.host || parsed.network}/32;\n`
            } else if (parsed.kind === "network") {
              const prefix = wildcardToPrefix(parsed.wildcard)
              output += `                    source-address ${parsed.network}/${prefix};\n`
            }
          } catch {
            output += `                    /* Error: Invalid source network */\n`
          }
        }
        output += `                }\n`
      } else {
        const extRule = rule as ExtendedACLRule
        output += `                from {\n`

        // Source
        if (extRule.sourceNetwork !== "any") {
          try {
            const parsed = parseACLNetwork(extRule.sourceNetwork)
            if (parsed.kind === "host") {
              output += `                    source-address ${parsed.host || parsed.network}/32;\n`
            } else if (parsed.kind === "network") {
              const prefix = wildcardToPrefix(parsed.wildcard)
        ...
        // Destination
        if (extRule.destNetwork !== "any") {
          try {
            const parsed = parseACLNetwork(extRule.destNetwork)
            if (parsed.kind === "host") {
              output += `                    destination-address ${parsed.host || parsed.network}/32;\n`
            } else if (parsed.kind === "network") {
              const prefix = wildcardToPrefix(parsed.wildcard)
              output += `                    destination-address ${parsed.network}/${prefix};\n`
            }
          } catch {
            output += `                    /* Error: Invalid destination network */\n`
          }
        }

        // Protocol
        if (extRule.protocol !== "ip" && extRule.protocol !== "any") {
          output += `                    protocol ${extRule.protocol};\n`
        }

        // Ports
        if ((extRule.protocol === "tcp" || extRule.protocol === "udp") && extRule.destPort) {
          if (extRule.destPortOperator === "range" && extRule.destPortRange) {
            const [start, end] = extRule.destPortRange.split("-").map((p) => p.trim())
            output += `                    destination-port ${start}-${end};\n`
          } else {
            output += `                    destination-port ${extRule.destPort};\n`
          }
        }

        // TCP established
        if (extRule.protocol === "tcp" && extRule.established) {
      ...
      if (rule.action === "permit") {
        ...
        output += `                    discard;\n`
        if ((rule as ExtendedACLRule).log || (rule as StandardACLRule).log) {
          output += `                    log;\n`
        }
      }
      output += `                }\n`
    ...
    output += `/* set interfaces ge-0/0/0 unit 0 family inet filter input ${aclName} */\n`
    output += `/* set interfaces ge-0/0/0 unit 0 family inet filter output ${aclName} */\n`

    return output
  }

  // Palo Alto PAN-OS Security Rule Generator
  const generatePaloAltoSecurityRules = (): string => {
    const timestamp = new Date().toISOString()
    const rules = aclType === "standard" ? standardRules : extendedRules
    let output = ""

    output += `# ${aclType.toUpperCase()} Security Policy ${aclName} - Generated by Network Toolbox\n`
    output += `# Generated on: ${timestamp}\n`
    output += `# Total rules: ${rules.length}\n`
    output += `# Format: PAN-OS CLI set commands\n\n`

    const rulebasePath = `set rulebase security rules`
    ...
    rules.forEach((rule, index) => {
      ...
      if (aclType === "standard") {
        ...
        output += `${rulebasePath} "${ruleName}" from any\n`
        output += `${rulebasePath} "${ruleName}" to any\n`

        // Source
        if (stdRule.sourceNetwork === "any") {
          output += `${rulebasePath} "${ruleName}" source any\n`
        } else {
          try {
            const parsed = parseACLNetwork(stdRule.sourceNetwork)
            if (parsed.kind === "host") {
              output += `${rulebasePath} "${ruleName}" source ${parsed.host || parsed.network}/32\n`
            } else if (parsed.kind === "network") {
              const prefix = wildcardToPrefix(parsed.wildcard)
              output += `${rulebasePath} "${ruleName}" source ${parsed.network}/${prefix}\n`
            }
          } catch {
            output += `# Error: Invalid source network\n`
          }
        ...
        output += `${rulebasePath} "${ruleName}" destination any\n`
        output += `${rulebasePath} "${ruleName}" application any\n`
        output += `${rulebasePath} "${ruleName}" service any\n`
      } else {
        const extRule = rule as ExtendedACLRule
        ...
        if (extRule.destNetwork === "any") {
          ...
          try {
            const parsed = parseACLNetwork(extRule.destNetwork)
            if (parsed.kind === "host") {
              output += `${rulebasePath} "${ruleName}" destination ${parsed.host || parsed.network}/32\n`
            } else if (parsed.kind === "network") {
              const prefix = wildcardToPrefix(parsed.wildcard)
              output += `${rulebasePath} "${ruleName}" destination ${parsed.network}/${prefix}\n`
            }
          } catch {
            output += `# Error: Invalid destination network\n`
          }
        ...
        // Application/Service
        if (extRule.protocol === "tcp" || extRule.protocol === "udp") {
          if (extRule.destPort) {
            output += `${rulebasePath} "${ruleName}" service [ service-${extRule.protocol}-${extRule.destPort} ]\n`
          } else {
            output += `${rulebasePath} "${ruleName}" service any\n`
          }
        } else if (extRule.protocol === "icmp") {
          output += `${rulebasePath} "${ruleName}" application [ ping ]\n`
          output += `${rulebasePath} "${ruleName}" service application-default\n`
        } else {
          output += `${rulebasePath} "${ruleName}" application any\n`
          output += `${rulebasePath} "${ruleName}" service any\n`
        }
      ...
      // Action
      output += `${rulebasePath} "${ruleName}" action ${rule.action === "permit" ? "allow" : "deny"}\n`

      // Logging
      if ((rule as ExtendedACLRule).log || (rule as StandardACLRule).log) {
        output += `${rulebasePath} "${ruleName}" log-start yes\n`
        output += `${rulebasePath} "${ruleName}" log-end yes\n`
      }

      // Description
      output += `${rulebasePath} "${ruleName}" description "${description}"\n`
      output += `\n`
    })

    // Implicit deny rule
    output += `# Implicit Deny Rule\n`
    output += `${rulebasePath} "${aclName}-deny-all" from any\n`
    output += `${rulebasePath} "${aclName}-deny-all" to any\n`
    output += `${rulebasePath} "${aclName}-deny-all" source any\n`
    output += `${rulebasePath} "${aclName}-deny-all" destination any\n`
    output += `${rulebasePath} "${aclName}-deny-all" application any\n`
    output += `${rulebasePath} "${aclName}-deny-all" service any\n`
    output += `${rulebasePath} "${aclName}-deny-all" action deny\n`
    output += `${rulebasePath} "${aclName}-deny-all" log-end yes\n`
    output += `${rulebasePath} "${aclName}-deny-all" description "Implicit deny all"\n\n`

    output += `# Don't forget to commit: commit\n`
  ...
  const generateACL = (): string => {
    ...
    if (platform === "juniper-junos") {
      return generateJuniperFirewallFilter()
    }

    if (platform === "paloalto-panos") {
      return generatePaloAltoSecurityRules()
    }
  ...
  const handleLoadFromProject = (data: Record<string, unknown>, item: ProjectItem) => {
    ...
    const savedAclType = data.aclType as "standard" | "extended" | undefined
    const savedPlatform = data.platform as string | undefined
